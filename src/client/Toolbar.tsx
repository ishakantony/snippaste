...
function Ico({ name, size = 14 }: { name: keyof typeof PATHS; size?: number }) {
  return (
...
function SavePill({ state }: { state: AutosaveState }) {
    ...
    <div className="save-pill">
      ...
      <span className="save-pill-text">
        {state.status === "saving" || state.status === "dirty"
          ? "saving…"
          : state.status === "saved"
          ? `auto-saved ${relativeTime(state.timestamp)}`
...
function ToolBtn({
  ...
  return (
    <button
      className={`toolbar-btn${danger ? " toolbar-btn--danger" : ""}`}
      onClick={onClick}
      title={label}
    >
...
export function Toolbar({ slug, saveState, onGetContent, onClear, onRefresh, onSave, remoteChanged }: ToolbarProps) {
  ...
  function handleCopyContent() {
    navigator.clipboard.writeText(onGetContent()).then(() => showToast("Copied"), () => {});
  }

  const isDirty = saveState.status === "dirty";

  return (
    <div className="toolbar">
      {/* Left — home + breadcrumb */}
      <div className="toolbar-left">
        <Link to="/" className="toolbar-home-btn" title="Home">
        ...
        {remoteChanged && (
          <span className="toolbar-remote-hint">
            · remote update —{" "}
            <button className="toolbar-inline-btn" onClick={onRefresh}>refresh</button>
          </span>
        )}
      </div>

      {/* Center — autosave pill (absolute) */}
      ...
      <div className="toolbar-right">
        <div className="toolbar-group">
          <ToolBtn icon="link"    label="Copy URL" onClick={handleCopyUrl} />
          <div className="toolbar-group-sep" />
          <ToolBtn icon="copy"    label="Copy"     onClick={handleCopyContent} />
          <div className="toolbar-group-sep" />
          <ToolBtn icon="save"    label="Save"     onClick={onSave} />
        </div>

        <div className="toolbar-group">
          <ToolBtn icon="trash"   label="Clear"   onClick={onClear}   danger />
          <div className="toolbar-group-sep" />
          <ToolBtn icon="refresh" label="Refresh" onClick={onRefresh} />
        </div>
