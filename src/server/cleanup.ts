...
export async function runCleanup(store: SnipStore): Promise<void> {
  try {
    ...
    console.log(`[cleanup] Removed ${deleted} stale snip(s) older than ${MAX_AGE_DAYS} days`);
  } catch (error) {
...
export function startCleanupJob(store: SnipStore): CronJob {
  ...
  console.log(`[cleanup] Scheduled daily cleanup at ${CRON_EXPRESSION} (server timezone)`);
  return job;
}
