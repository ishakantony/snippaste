...
import { AutosaveController, type AutosaveState, type ClearTimeoutLike, type SetTimeoutLike } from "../src/client/autosaveController.js";
...
function timerDeps() {
  const st: SetTimeoutLike = (fn, ms) => globalThis.setTimeout(fn, ms) as unknown as number;
  const ct: ClearTimeoutLike = (id) => globalThis.clearTimeout(id as unknown as Parameters<typeof globalThis.clearTimeout>[0]);
...
describe("AutosaveController", () => {
  ...
  it("saved state carries a wall-clock timestamp", async () => {
    ...
    expect(savedState).toEqual({ status: "saved", timestamp: 1_700_000_000_000 });
  });
  ...
  it("retry-on-next-change: Offline → Dirty → Saving → Saved after recovery", async () => {
    ...
    const { controller, states } = makeController([{ ok: false }, { ok: true }]);
  ...
  it("no concurrent in-flight saves: onChange during Saving queues pending, not a second fetch", async () => {
    ...
    const fakeFetch = vi.fn((_url: string, _init: RequestInit) => firstFetchPromise);
    const states: AutosaveState[] = [];
