...
describe("snipStream.subscribe", () => {
  it("opens an EventSource at /api/snips/<slug>/events", () => {
    const h = makeHarness();
    subscribe("my-slug", {} as SnipStreamHandlers, { eventSourceFactory: h.factory });
  ...
  it("encodes the slug in the URL", () => {
    const h = makeHarness();
    subscribe("a-b-c", {} as SnipStreamHandlers, { eventSourceFactory: h.factory });
