...
describe("SlugValidator.validate", () => {
	...
	const validCases: Array<{
		...
		{ input: "a", expectedSlug: "a", label: "length 1 (min)" },
		{
			...
			label: "mid-range with hyphen",
		},
		{
			input: "abc123",
			expectedSlug: "abc123",
			label: "alphanumeric mid-range",
		},
		{
			input: "a".repeat(64),
			expectedSlug: "a".repeat(64),
			label: "length 64 (max)",
		},
		{
			input: "  hello  ",
			expectedSlug: "hello",
			label: "whitespace trimmed to valid slug",
		},
		{
			input: "UPPER-CASE",
			expectedSlug: "upper-case",
			label: "uppercase input normalized to lowercase",
		},
	];
